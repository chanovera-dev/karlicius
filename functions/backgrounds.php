    ...
    ?>
        <style>
            ...
            <?php else: ?>
                ...
                :is(.home,.single-post,.page-template-default) #main #hero.background-hero.container{background:linear-gradient(to bottom, rgba(0,0,0,.7), rgba(0,0,0,0)), url('<?php the_post_thumbnail_url( 'full' ); ?>');}
                :is(.home,.single-post,.page-template-default) #main :is(.header-post,.background-hero).container .background-blur{position:absolute;top:0;left:0;width:100%;height:100%;z-index:0;}
